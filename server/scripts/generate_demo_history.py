#!/usr/bin/env python3
"""
Regenerate demo analytics history inside server/CCoFSchema.sql.

Run from server/:
  python scripts/generate_demo_history.py

Re-seeding via seed_redis.py applies CCoFSchema.sql (schema + live seed + history).
Redis stays light (live queue only: entries 1–3 from the base schema).
"""
...
BEGIN_MARKER = "-- BEGIN DEMO HISTORY (generated by scripts/generate_demo_history.py — do not edit manually)"
END_MARKER = "-- END DEMO HISTORY"
...
def build_demo_history_sql(registration_id: int, entry_id: int) -> tuple[list[str], int, int]:
    ...
    return lines, registration_id, entry_id


def patch_schema(demo_lines: list[str]) -> None:
    schema = SCHEMA_PATH.read_text()
    if BEGIN_MARKER not in schema or END_MARKER not in schema:
        raise ValueError(f"Markers not found in {SCHEMA_PATH}")

    before, rest = schema.split(BEGIN_MARKER, 1)
...
def main() -> None:
    demo_lines, next_registration_id, next_entry_id = build_demo_history_sql(
        FIRST_REGISTRATION_ID,
        FIRST_ENTRY_ID,
    )
    patch_schema(demo_lines)

    registration_count = next_registration_id - FIRST_REGISTRATION_ID
    entry_count = next_entry_id - FIRST_ENTRY_ID
    print(f"Updated {SCHEMA_PATH}")
    print(f"  demo registrations: {registration_count} (ids {FIRST_REGISTRATION_ID}–{next_registration_id - 1})")
    print(f"  demo queue entries: {entry_count} (ids {FIRST_ENTRY_ID}–{next_entry_id - 1})")
